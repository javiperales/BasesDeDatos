...
CREATE TABLE productos_proveedores (
	...
	dias_envio int,
	constraint prod_prov_pk primary key(id_prov, numero_producto),
...
INSERT INTO categorias
  VALUES (3, 'Ropa');
INSERT INTO categorias
  VALUES (4, 'Componentes');
INSERT INTO categorias
  VALUES (5, '**** para el coche');
INSERT INTO categorias
  VALUES (6, 'Ruedas');
INSERT INTO categorias
  VALUES (7, 'Scooters');
...
id_campañas number(3) constraint id_campañas_pk primary key,
titulo varchar2(20),
...
select nombre from empleados
  ...
  select id_empleado, nombre , apellidos from empleados
    where id_empleado not in (
      select id_empleado from pedidos 
        where id_cliente=(
          select id_cliente from clientes where nombre like 'Samuel' and apellidos like 'Abadia'));
...
select nombre ,ciudad from clientes
  where id_cliente not in  (
    select id_cliente from pedidos natural join  productos
...
--corregido
select id_empleado, nombre , sum(cantidad*precio) as total 
from detalles_pedidos natural join pedidos natural join 

/*5 corregido*/
select id_empleado, nombre, sum(precio_total)
  from empleados natural join pedidos
    where id_empleado =(
    select id_empleado
      from (select count(*) as total , id_empleado from pedidos
...
--6 corregido
select DISTINCT EMPLEADOS.NOMBRE,empleados.apellidos,clientes.cod_postal
from empleados join pedidos using(id_empleado)
join clientes using(id_cliente)
join DETALLES_PEDIDOS using(numero_pedido)
where precio between 500 and 1900;


select * from
(select pedidos.*
from pedidos join clientes on PEDIDOS.ID_CLIENTE=CLIENTES.ID_CLIENTE
where provincia in ('CACERES','MADRID') AND NOMBRE LIKE 'A%'
...
select * from (
  ...
  select * from (
    select pedidos.*  from pedidos join clientes on pedidos
...
select * from pedidos
  ...
  insert into pedidos_historicos
        select * from pedidos where fecha_envio in (
          select numero_pedido from pedidos_historico);
          
          insert into detalles_pedidos_historico
            select * from detalles_pedidos where
              numero_pedido in (select numero_pedido from pedidos_historico);
          
        delete from detalles_pedidos
          where numero_pedido in (
            select numero_pedido from pedidos_historico);
            
          detele from pedidos where numero_pedido
          in ( select numero_pedido from pedidos_historico);
...
select emple.id_empleado , emple.nombre , pe.precio_total from empleados emple , pedidos pe
  ...
  /*8*/
  select * from (
  select    emple.id_empleado , emple.nombre , count(pe.id_cliente)  from empleados emple , pedidos pe
  group by emple.id_empleado , emple.nombre
